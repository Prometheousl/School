...
static void growCDA(CDA *items);
static void shrinkCDA(CDA *items);
static void freeArray(int capacity, void **array);
/**
 ...
 */
static void shrinkCDA(CDA *items) {
	void **newArray = (void**)malloc(items->capacity / 2 * sizeof(void *));
	assert(newArray != 0);
	for(int i = 0; i < items->size; ++i) {
		newArray[i] = getCDA(items,i);
	}
...
/**
 * Frees the underlying array and CDA object
 *
 * @param items = The CDA object to be freed
 */
...
/**
 * Frees the underlying array
 *
 * @param capacity = CDA's capacity
