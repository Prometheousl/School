...
static void createProducerThreads(pthread_t *p_tid, int num) {
  for(int x = 0; x < num; x++) {
    pthread_create(&p_tid[x],NULL,producerFunction,NULL);
  }
}
static void createConsumerThreads(pthread_t *c_tid, int num) {
  for(int x = 0; x < num; x++) {
    pthread_create(&c_tid[x],NULL,consumerFunction,NULL);
  }
}
static void *producerFunction(void *p_tid) {
  ...
  if(p_tid) {}; // Stops a warning message at compile
  return NULL;
}
static void *consumerFunction(void *c_tid) {
  ...
  if(c_tid) {}; // Stops a warning message at compile
  return NULL;
}
